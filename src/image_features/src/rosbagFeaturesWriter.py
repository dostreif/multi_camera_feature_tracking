...
class FeaturePublisher:
    ...
    """
    def __init__(self):
        # define new bag to write to
        self.bag = rosbag.Bag('/media/dominic/Extreme SSD/datasets/rosbags/test.bag', 'w')
        # define bag to be loaded
        self.loadedBag = rosbag.Bag('/media/dominic/Extreme SSD/datasets/rosbags/thurbo_2018-10-25-10-43-25_chunk.bag')
        ...
        self.image_topic0 = '/bas_usb_0/image_raw'
        self.image_topic1 = '/bas_usb_1/image_raw'
        ...
        self.image_timestamps = [None, None]
    ...
    def image_handler(self, img, cam_id):
        ...
        if not self.image_avail[cam_id]:
            if self.image_timestamps[cam_id] is None or rospy.Time.to_sec(img.header.stamp) - rospy.Time.to_sec(self.image_timestamps[cam_id]) > 1/self.rate:
                print('set cam%u to true' % cam_id)
                self.image_timestamps[cam_id] = img.header.stamp
    ...
    def get_features(self):
        ...
        # publish messages
        self.publish_feature_msg(pts[0].squeeze(), descriptors[0].squeeze(), self.images[0, :, :].squeeze(), self.topic0, self.image_timestamps[0])
        self.publish_feature_msg(pts[1].squeeze(), descriptors[1].squeeze(), self.images[1, :, :].squeeze(), self.topic1, self.image_timestamps[1])
        print("published")
        self.image_avail = [False, False]
        self.processing = False
        self.rosRate.sleep()
    ...
    def run(self):
        for topic, msg, t in self.loadedBag.read_messages():
            if topic == self.image_topic0:
                print(topic)

                self.callback_cam0(msg)
            elif topic == self.image_topic1:
                print(topic)
                self.callback_cam1(msg)
            self.bag.write(topic, msg, t=t)
        self.bag.close()
    ...
    def publish_feature_msg(self, pts, descriptors, img, topic, stamp):
        """
